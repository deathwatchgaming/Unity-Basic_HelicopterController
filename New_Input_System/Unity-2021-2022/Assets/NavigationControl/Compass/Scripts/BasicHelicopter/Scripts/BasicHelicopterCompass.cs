/*
*  Name: Basic Helicopter Compass
*  File: BasicHelicopterCompass.cs
*  Author: Deathwatch Gaming
*  License: MITT
*  Unity Version(s): Unity 20221+ - 2022+
*/
...
namespace NavigationControl
{
	// public class BasicHelicopterCompass
	public class BasicHelicopterCompass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Basic Helicopter transform")]
			// Transform _basicHelicopter
			[SerializeField] private Transform _basicHelicopter;
		...
		// BasicHelicopterCompass _basicHelicopterCompass
		public static BasicHelicopterCompass _helicopterCompass;

		// private void Start
		private void Start()
		{
			// _helicopterCompass
			_helicopterCompass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component BasicHelicopterCompass is enabled
				GetComponent<BasicHelicopterCompass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component BasicHelicopterCompass is not enabled
				GetComponent<BasicHelicopterCompass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _basicHelicopter localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_basicHelicopter.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the basic helicopter forward vector

			// Vector3 forward is _basicHelicopter transform forward
			Vector3 forward = _basicHelicopter.transform.forward;

			// Zero out the Y of the basic helicopter forward vector to only get the direction in the X,Z 
	...
	} // close public class BasicHelicopterCompass
